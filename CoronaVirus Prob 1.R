...
death_world <- read.csv("time_series_covid19_deaths_global1.csv",stringsAsFactors = FALSE,
                        check.names =  FALSE)

recovered_world <- read.csv("time_series_covid19_recovered_global1.csv",stringsAsFactors = FALSE,
                            check.names =  FALSE)
...
death_world <- reshape2::melt(death_world, id.vars = c("Province/State", "Country/Region","Population", "Lat", "Long"), variable.name = "Date", value.name = "Death")

recovered_world <- reshape2::melt(recovered_world, id.vars = c("Province/State", "Country/Region","Population", "Lat", "Long"), variable.name = "Date", value.name = "Recovered")
...
world_history_data <- dplyr::left_join(confirmed_world, death_world, by = c("Province/State", "Country/Region","Population", "Lat", "Long", "Date"))

head(world_history_data)

world_history_data <- dplyr::left_join(world_history_data, recovered_world, by = c("Province/State", "Country/Region","Population", "Lat", "Long", "Date"))
...
world.summary.data <- ddply(world_history_data,.(Country.Region, Date),function(x){
    colSums(x[,c("Population","Confirmed","Death","Recovered")])
})
...
ggplot(major.summary.data[major.summary.data$Country.Region != "China", ],
    aes(x = Date, y = Infectedpermil, col = Country.Region)) + geom_line(lwd = 1) + geom_point(size = 2) +
